...
- Latest verification: `verify-system.ps1` requires at least 21 pack manifests, 21 generated pack download ZIPs, the marketplace import kit, topic pages/topic JSON, 4 policy pages, JSON/RSS/Atom feed paths, social metadata on generated public pages, the GitHub fallback workflow, the daily task, and the watchdog task. It passed on 2026-06-02 after feed commit `cd2dd12` with `files_checked: 30`. Live checks passed for `feed.xml` and `atom.xml`; IndexNow accepted 6 refreshed feed/discovery URLs with HTTP 200 and a follow-up dry run showed 0 queued URLs. GitHub fallback run `26803952952` logged `No generated changes to publish`.
