...
- Latest verification: `verify-system.ps1` requires at least 21 pack manifests, 21 generated pack download ZIPs, the marketplace import kit, topic pages/topic JSON, offer pages/offer JSON, 4 policy pages, support page, `llms.txt`, `llms-full.txt`, JSON/RSS/Atom feed paths, social metadata on generated public pages, the public support destination when support mode is connected, the GitHub fallback workflow, the daily task, and the watchdog task. It passed on 2026-06-02 after offer-page commit `5d6eec6` with `files_checked: 36`, `bundle_bytes: 323728`, `pack_download_bytes: 126854`, `store_import_zip_bytes: 161610`, `monetization_enabled: true`, `support_connected: true`, and `store_connected: false`. Pages run `26805912605` deployed the offer pages; browser QA passed locally for `offers/small-business-ops.html`; IndexNow accepted 21 changed URLs and a follow-up dry run showed 0 queued URLs. GitHub fallback run `26805969926` logged `No generated changes to publish`. CalmSprout Worker commit `f063a55` deployed directly to Cloudflare as version `cb0233b3-ec1d-4599-b445-417ee04f610e`; live `www.calmsprout.com/daily-shelf*` routes redirect to the shelf/offers/Square support path and the `www.calmsprout.com` homepage contains the shelf banner.
