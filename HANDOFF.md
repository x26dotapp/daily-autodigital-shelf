...
- CalmSprout bridge: `C:\scripts\CalmSprout` commits `f063a55` (`Add Daily Shelf bridge`), `d8fa6d9` (`Add Daily Shelf landing page`), `8a689e1` (`Add CalmSprout IndexNow key`), `8d4d298` (`Add Daily Shelf pay bridge`), `49b00c2` (`Add Daily Shelf LLM discovery`), `9401218` (`Add Daily Shelf bridge aliases`), `0c509a0` (`Add Daily Shelf data aliases`), `6786fe0` (`Add dynamic Daily Shelf current pack routes`), `557e6c5` (`Add Daily Shelf product catalog page`), `9194d08` (`Add Daily Shelf product detail routes`), `d8af561` (`Add Daily Shelf dynamic product sitemap`), `3001f49` (`Add Daily Shelf product support funnels`), and `6b5e7bf` (`Add Daily Shelf support intent redirects`) deployed directly to Cloudflare. Latest Worker version is `f571cf59-c42e-4781-ad71-7fc8e2861646`. Live `https://www.calmsprout.com/daily-shelf` and `/daily-shelf/today` now render the current pack from live shelf status; `/daily-shelf/products`, `/daily-shelf/browse`, `/daily-shelf/products/<pack-slug>`, and `/daily-shelf/products/<pack-slug>/support` render the live catalog as first-party catalog/product/support pages; `/daily-shelf/products/<pack-slug>/support/go` redirects product support intent to Square with CalmSprout UTM attribution; `/sitemap.xml` now includes catalog-derived product detail/support URLs and `/daily-shelf/product-sitemap.xml` exposes product and support URLs; `/daily-shelf/packs/<pack-slug>/...`, `/daily-shelf/today.zip`, `/daily-shelf/current.zip`, and `/daily-shelf/downloads/<pack>.zip` proxy public pack pages/assets/ZIPs; `/daily-shelf/pay` and `/daily-shelf/bundle` are first-party support funnel pages; `/daily-shelf/offers` redirects to the offer index; `/daily-shelf/support` redirects to the public Square support page; `/daily-shelf/status`, `/daily-shelf/catalog.json`, `/daily-shelf/catalog.csv`, `/daily-shelf/store-listings.json`, `/daily-shelf/store-listings.csv`, `/daily-shelf/feed.json`, `/daily-shelf/feed.xml`, `/daily-shelf/atom.xml`, and `/daily-shelf/starter.zip` proxy public shelf data/assets under the CalmSprout domain; `robots.txt`, `sitemap.xml`, `llms.txt`, `llms-full.txt`, and public IndexNow key file `a4f604db6d2046939ff6c7e3d29d341e.txt` are live; the `www.calmsprout.com` homepage includes Daily Shelf banner links for offers, products, today, bundle, pay, and support.
...
- Discovery: IndexNow key files are live for both the GitHub Pages shelf and CalmSprout branded bridge. Latest direct `tools/submit_indexnow.py --all --wait-for-key-seconds 90` submitted 8 support-metadata URLs and received HTTP 200. Latest direct `tools/submit_calmsprout_indexnow.py --all --wait-for-key-seconds 90` submitted 21 CalmSprout product support URLs with HTTP 200; follow-up dry runs returned `submit_count: 0`.
...
- Latest functional infrastructure commit: `41949c7` (`Track CalmSprout product support routes`) pushed to `main`; GitHub Pages deployment `26812672540` succeeded
