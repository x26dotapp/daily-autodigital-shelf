...
- Discovery: IndexNow key files are live for both the GitHub Pages shelf and CalmSprout branded bridge. Latest direct `tools/submit_indexnow.py --all --wait-for-key-seconds 90` submitted 8 support-metadata URLs and received HTTP 200. Latest direct `tools/submit_calmsprout_indexnow.py --all --wait-for-key-seconds 90` submitted 21 CalmSprout product detail URLs and received HTTP 200; follow-up dry runs returned `submit_count: 0`.
...
- Latest functional infrastructure commit: `cc21a51` (`Track CalmSprout product detail routes`) pushed to `main`; GitHub Pages deployment `26811768255` succeeded
