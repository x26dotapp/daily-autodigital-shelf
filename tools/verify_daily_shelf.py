...
def verify_local(day: str, min_pack_count: int = 1) -> dict[str, Any]:
    ...
    require_contains(
        ...
        ["/daily-shelf/today.zip", "/daily-shelf/current.zip", "/daily-shelf/products", "/daily-shelf/products/", "/daily-shelf/products/{slug}/support", "/daily-shelf/product-sitemap.xml"],
    )
    require_contains(
        ROOT / "run-daily.ps1",
